...
from cpshift.data_v10 import generate_source_split, generate_target, oracle_density_ratio
...
class MeanConformal:
    def __init__(self,kind:str,seed:int=0):
        self.kind=kind
        if kind=='Ridge': self.model=Ridge(alpha=1.0)
        elif kind=='GBR': self.model=GradientBoostingRegressor(random_state=int(seed),**GBR_PARAMS)
        elif kind=='HGBR': self.model=HistGradientBoostingRegressor(random_state=int(seed),**PUBLIC_HGBR_PARAMS)
...
def rec(rows,base,method,lo,hi,metrics): rows.append({**base,'method':method,**metrics})
...
def path_areas(df,groups):
    ...
    for key,g in df.groupby(groups+['method'],dropna=False):
        ...
        for col,name in [('coverage_deficit','A_cov'),('coverage_gap','A_gap'),('mean_width_finite','A_width'),('interval_score_finite','A_score'),('infinite_interval_fraction','A_inf')]:
            y=g[col].to_numpy(float); r[name]=float(np.trapezoid(y,x)) if np.isfinite(y).all() else np.nan
        outs.append(r)
    ...
    for rep in range(int(rep_start),end):
      for di,dgp in enumerate(cfg['dgp_types']):
       for ni,noise in enumerate(cfg['noise_types']):
        ...
        ridge=MeanConformal('Ridge',mseed).fit(Xtr,ytr,Xcal,ycal); gbr=MeanConformal('GBR',mseed).fit(Xtr,ytr,Xcal,ycal)
        cqr_gbr=ConformalizedQuantileRegressor(alpha=ALPHA,n_estimators=int(cfg['cqr_estimators']),random_state=mseed).fit(Xtr,ytr,Xcal,ycal); cqr_lin=CQRLinear().fit(Xtr,ytr,Xcal,ycal)
...
def run_public(rep_start=0,rep_end=None,datasets=None,modes=('directional','radial')):
    ...
    for key in datasets:
      ...
      for rep in range(int(rep_start),end):
        ...
        seed=derive_seed(master,didx,rep,0,20); ridge=MeanConformal('Ridge',seed).fit(Xtr_s,ytr_s,Xcal_s,ycal_s); hgb=MeanConformal('HGBR',seed).fit(Xtr_s,ytr_s,Xcal_s,ycal_s)
        for mode in modes:
            ...
            for si,sv in enumerate(sevs):
                ...
                for name,obj in [('SCP-Ridge',ridge),('SCP-HGBR',hgb)]:
                ...
                for name,obj in [('Known-tilt-WCP-Ridge',ridge),('Known-tilt-WCP-HGBR',hgb)]:
...
def factorial_contrasts(areas,layer):
    rows=[]; group_cols=['dgp','noise','scenario'] if layer=='synthetic' else ['dataset','scenario']
    for key,g in areas.groupby(group_cols,dropna=False):
        ...
        if layer=='synthetic': wr,wg='Oracle-WCP-Ridge','Oracle-WCP-GBR'; second='SCP-GBR'
        else: wr,wg='Known-tilt-WCP-Ridge','Known-tilt-WCP-HGBR'; second='SCP-HGBR'
        for metric in ['A_cov','A_gap','A_width']:
            P=piv[metric]
            specs=[('Backbone effect within SCP',second,'SCP-Ridge'),('Backbone effect within WCP',wg,wr),('Wrapper effect within Ridge',wr,'SCP-Ridge'),('Wrapper effect within second backbone',wg,second)]
            ...
            if all(x in P for x in [wg,second,wr,'SCP-Ridge']):
                d=(P[wg]-P[second])-(P[wr]-P['SCP-Ridge']); n,m,lo,hi=paired_ci(d.dropna()); rows.append({'layer':layer,**meta,'metric':metric,'contrast':'Wrapper x backbone interaction','n':n,'mean_difference':m,'ci95_low':lo,'ci95_high':hi})
...
def main():
    ...
    if a.smoke: syn=run_synthetic(0,1); pub=run_public(0,1,['ccpp'])
    else: syn=run_synthetic(a.syn_start,a.syn_end); pub=run_public(a.pub_start,a.pub_end,a.datasets)
